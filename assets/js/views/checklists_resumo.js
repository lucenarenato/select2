{
    tagName: "div",
    id: "adagio-home",
    className: "adagio-checklists-resumo",
    events: {
        "submit": "updating"
    },
    updating: function(event)
    {
        ...
        $.ajax({
            url: "/interfaces/checklists/" + vars.id,
            method: "patch",
            data: $dom.serialize(),
            dataType: "json",
...
var ocorrenciasId = _.union(
    this.model.get('ocorrencias').pluck('problema_id'),
    this.model.get('ocorrencias_antigas').pluck('problema_id')
);
...
var extintosId = _.difference(
    ...
    this.model.get('ocorrencias').pluck('problema_id')
        ...
        this.model.get('ocorrencias').each(function (dado) {
            var ocorrencia = this.model.get('ocorrencias_antigas').findWhere({problema_id: dado.get('problema_id')}),
            ...
            <tr class=`+(taxa==='0.00'?'warning':'')+`>
                <td>
                    <i class="fa fa-circle `+inspecoesPorOcorrencia.niveis[dado.get('nivel')]+` margin-right-5"></i>&nbsp;<strong>`+dado.get('titulo')+`</strong>&nbsp;<small class="visible-lg-inline-block">`+inspecoesPorOcorrencia.tensoes[dado.get ('nivel')]+`</small>
                </td>
        ...
        _(extintosId).each(function(item) {
            var ocorrencia = this.model.get('ocorrencias_antigas').findWhere({problema_id: item}).attributes,
